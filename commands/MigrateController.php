...
class MigrateController extends Command
{
    ...
    public function actionUp(): int
    {
        ...
        if ($this->verbose > 2) {
            echo "Migrating from path '$migrationPath'", PHP_EOL;
        }
        $dh = opendir($migrationPath);
        if (!$dh) {
            throw new Exception("Invalid dir " . $migrationPath);
        }
        $new = [];
        while (($file = readdir($dh)) !== false) {
            if (filetype($migrationPath . '/' . $file) == 'file' && preg_match(
        ...
        if ($this->verbose) {
            ...
            if ($this->confirm != 'yes' && !CliHelper::confirm('Apply all the new migrations?')) {
                exit;
            }
        }
        ...
        foreach ($new as $file) {
            ...
            try {
                if ($this->verbose > 1) {
                    echo "Applying $name from $filename", PHP_EOL;
                }
                if ($ext == 'php') {
                    $success = false;
                    ...
                    try {
                        ...
                        $migration = new $className(
                            ['app' => $this->app, 'connection' => $this->connection, 'verbose' => $this->verbose]
                        );
                    ...
                    $migration = new SqlMigration(
                        ['connection' => $this->connection, 'filename' => $filename, 'verbose' => $this->verbose]
                    );
                    $success = $migration->up();
    ...
    public function createMigrationTable(): bool
    {
        ...
        if (!$this->connection->tableExists($this->migrationTable)) {
            if ($this->confirm != 'yes' && !CliHelper::confirm(
                    "The `$this->migrationTable` table does not exist. Create?"
                )) {
                exit;
            }
            echo "Creating `migration` table...", PHP_EOL;
            $this->connection->pdo->query(
                'CREATE TABLE ' . $this->migrationTable . ' (name varchar(100) unique primary key, applied int)'
            );
            if (!$this->connection->tableExists($this->migrationTable)) {
     ...
     * @return bool -- success
     */
    public function createMigrationPath(): bool
    {
        if (!is_dir($this->migrationPath)) {
            if ($this->confirm != 'yes' && !CliHelper::confirm('The `/migrations` directory does not exist. Create?')) {
                exit;
            }
     ...
     */
    public function createMigration(string $name): int
    {
        if (!$name) {
        ...
        if ($this->confirm != 'yes' && !CliHelper::confirm("Create new migration '$file'?")) {
            exit(-1);
        }
        $content = <<<EOT
<?php
...
EOT;
    ...
    {
        if ($this->confirm != 'yes' && !CliHelper::confirm('Are you sure to delete the whole database?')) {
            return 1;
        }
     ...
     * @throws Exception
     */
    public function truncateDatabase(int $verbose = 0): bool
    {
        ...
        foreach ($metadata as $tableName => $tableData) {
            ...
            if ($foreignKeys) {
                foreach ($foreignKeys as $name => $foreignKey) {
                    ...
                    if (!$this->connection->dropForeignKey($constraint_name, $table_name)) {
                        $success = false;
                    } elseif ($verbose > 1) {
                        echo "Foreign key $name dropped.\n";
                    }
                }
            }
        }
        ...
        foreach ($metadata as $tableName => $schema) {
            if ($verbose > 2) {
                echo "Dropping table/view '$tableName'\n";
            }
            try {
                ...
                if ($this->connection->isViewRelated($message = $e->getMessage())) {
                    if (!$this->connection->dropView($tableName)) {
                        $success = false;
                    } elseif ($verbose > 1) {
                        echo "View $tableName dropped.\n";
                    }
                } else {
        ...
        foreach ($functions as $functionName) {
            if ($verbose > 2) {
                echo "Dropping $functionName\n";
            }
            if ($this->connection->dropRoutine($functionName)) {
                if ($verbose > 1) {
                    echo "$functionName dropped.\n";
                }
            } else {
        ...
        if ($verbose > 1 && $sequences) {
            echo "Dropping sequences...\n";
        }
        foreach ($sequences as $sequenceName) {
            if ($verbose > 2) {
                echo "Dropping sequence $sequenceName\n";
            }
            if (!$this->connection->dropSequence($sequenceName)) {
                $success = false;
            } elseif ($verbose > 1) {
                echo "Sequence $sequenceName dropped.\n";
            }
        }
     ...
     */
    public function actionWait(int $timeout = 60, int $interval = 5): int
    {
