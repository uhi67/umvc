...
$app_config = include dirname(__DIR__, 2) .'/config.php';

$env = $app_config['application_env'] ?? 'development';
if($env!='production') ini_set('display_errors', 'On');

$dataDir = dirname(__DIR__, 3) .'/runtime/saml';
if(!is_dir($dataDir)) mkdir($dataDir, 0770);
...
$config = [
	...
	'loggingdir' => dirname(__DIR__,3).'/runtime/logs/',
	'datadir' => $dataDir.'/',
	 ...
	 */
	'auth.adminpassword' => $app_config['saml_admin_password'],
	 ...
	 */
	'secretsalt' => $app_config['saml_secret_salt'],
	...
	'technicalcontact_email'    => 'support@mvc.test',
	 ...
	 *
	 */
	'logging.level' => $env=='development' ? Logger::NOTICE : Logger::INFO,
	 ...
	 */
	'enable.saml20-idp' => $env!='production',
	 ...
	 */
	'session.cookie.name' => 'SimpleSAMLSession_credit',
	 ...
	 */
	'session.authtoken.cookiename' => 'SimpleSAMLAuthToken_credit',
	...
	'idpdisco.rememberchecked' => $env=='production',
	...
	'authproc.idp' => [
		/*
			10 => array(
				...
				'not_configured_url' => 'http://uhi.test/simplesaml/module.php/simpletotp/not_configured.php',
			),
			*/
	...
	'metadata.sources' => [
        ...
        [
            'type' => 'flatfile',
            'directory' => dirname(__DIR__, 2) .'/saml/metadata/private',
        ],
		[
			'type' => 'mdx',
			'server' => 'https://mdx.eduid.hu',
			'cachedir' => dirname(__DIR__, 2) .'/runtime/mdx-cache', // optional, but recommended
			'cachelength' => 86400, // optional
	 ...
	 * syntaxes.
	 */
	'store.sql.dsn'                 => 'sqlite:'.$dataDir.'/simplesaml.sq3',
	...
	'metadata.sign.privatekey' => 'mvc.pem',
	'metadata.sign.privatekey_pass' => NULL,
	'metadata.sign.certificate' => 'mvc.crt',
