...
class Assertions {
	...
	static function assertString($x, $null=false) {
		if($null && $x===null) return;
		if(!is_string($x) && !is_numeric($x)) throw new Exception('#1:parameter must be string, got ' . static::typeName($x));
	}
	...
	static function assertInt($x) {
		if(!is_scalar($x) || strval($x) != strval(intval($x))) throw new Exception('#1:parameter must be int, got ' . static::typeName($x));
	}
	...
	static function typeName($value) {
		if(!is_object($value)) return gettype($value);
