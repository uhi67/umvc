...
class Query extends Component
{
    ...
    private function allModelFields(array $models): array
    {
        ...
        foreach ($models as $key => $modelName) {
            if ($modelName instanceof Query) {
                ...
                if (is_integer($key) && $forceAlias) {
                    $a1 = $modelName::tableName();
                } else {
                    $a1 = $key;
                }
                $fields1 = $this->modelFields($modelName);
                if ($a1) {
                    $fields1 = array_map(function ($f) use ($a1) {
                        return $a1 . '.' . $f;
                    }, $fields1);
                }
    ...
    public function addSelect(array $expressionList = null): static
    {
        if ($expressionList) {
            if (!$this->_fields) {
                $this->_fields = $this->allModelFields($this->_from);
            }
    ...
    public function join(array $joinList): static
    {
        ...
        if (isset($joinList[0]) && !is_array($joinList[0])) {
            $joinList = [$joinList];
        }
    ...
    public function asModel(Model|string $modelClass): static
    {
        if (!is_a($modelClass, Model::class, true)) {
            throw new Exception('modelClass must be a Model');
        }
    ...
    public function getAll(): ?array
    {
        try {
            ...
            if (ENV_DEV) {
                throw new Exception($e->getMessage() . ' while executing SQL: ' . $this->sql, 500, $e);
            } else {
                throw $e;
            }
        }
    ...
    public function getOne(): array|Model|null
    {
        ...
        if ($result === false && !str_starts_with($error, '00000')) {
    ...
    public function getColumn(int|string $column = 0): array
    {
        ...
        while ($row = $this->stmt->fetch()) {
            if ($index && !isset($row[$index])) {
                throw new Exception("Invalid index column $index");
            }
            if (!$row[$index]) {
                throw new Exception(
                    "Empty index: " . json_encode(['column' => $column, 'row' => $row, 'index' => $index])
                );
            }
            if ($index) {
    ...
    {
        if ($column === 0 && $this->sql && $this->_scalar !== null) {
            return $this->_scalar;
        }
        $this->stmt = null;
        try {
            ...
            $details = ENV_DEV ? '; ' . $phase . ' "' . $this->sql . '" with parameters [' . implode(
                    ',',
    ...
    {
        if ($this->type == 'SELECT') {
            throw new Exception('Use get*() methods for SELECT queries');
        }
        $this->stmt = $this->prepareStatement();
        try {
    ...
    {
        if (!$this->_connection) {
            $this->_connection = App::$app->getConnection(true);
        }
    ...
    public function into(string|array $modelClass, array $fieldList = null): static
    {
        ...
        if ($this->modelClass && is_array($modelClass) && $fieldList === null) {
            ...
            if (is_array($modelClass)) {
                throw new Exception('fieldList can be specified alone only if modelClass is already stored');
            }
            if (!is_a($modelClass, Model::class, true)) {
                throw new Exception('Invalid modelClass');
            }
            $this->modelClass = $modelClass;
            if ($fieldList) {
    ...
    public function prepareStatement(array $options = []): false|PDOStatement
    {
        $stmt = $this->connection->pdo->prepare(
            $this->sql,
            $options
        ); // this actually builds up the SQL statement and computes the inner parameters
        if ($stmt === false) {
            return false;
        }
        if ($this->_innerParams) {
            foreach ($this->_innerParams as $name => &$value) {
                if (!$stmt->bindParam(is_int($name) ? $name + 1 : $name, $value)) {
                    return false;
                }
            }
        }
     ...
     * @throws Exception
     */
    public function modelFields(string|Model $modelName): array
    {
        if (!is_callable(array($modelName, 'databaseAttributes'))) {
            throw new Exception("Invalid model classname '$modelName'");
        }
...
// Return the alias of the main model
        ...
        if ($alias === false || is_int($alias)) {
            $alias = null;
        }
        return $alias;
    }

    public function findUniqueAlias(string $baseName, array $aliases): string
    {
        $alias = $baseName;
        $i = 1;
    ...
    {
        if (!is_array($models)) {
            return $this->buildTableName($models);
        }
    ...
    public function buildTableName(Query|string $model, string $alias = ''): string
    {
        ...
        return static::quoteFieldName($name) . (($alias && ($alias != $name)) ? (' ' . static::quoteFieldName(
                    $alias
                )) : '');
    }
    ...
    public function buildWhere($alias = null): string
    {
        ...
        if (is_null($condition) || is_array($condition) && count($condition) == 0) {
            return '';
        }
        if ($condition === true) {
            return '';
    ...
    public function buildExpression(mixed $expression, string $alias = null, int $precedence = null): float|int|string
    {
            ...
            if (strpos($expression, ' ')) {
                ...
                if (strtoupper($ee[1]) == 'AS' && count($ee) == 3) {
                    ...
                    if (count($ee) == 2) {
                        ...
                        throw new Exception(
                            sprintf('Invalid expression: too many whitespaces in field name `%s` ', $expression)
                        );
                    }
                }
            }
        ...
        if (is_array($expression)) {
            if (isset($expression[0])) {
                ...
                if (($ops = static::isOperator($op)) !== false) {
                    ...
                    if ($ops < 4 && count($expression) != $ops) {
                        throw new Exception("Operator `$opr` requires $ops operand");
                    }
                    switch ($ops) {
                        case 0:
                            return $opr;
                        ...
                        case 4:
                            ...
                            try {
                                ...
                                if ($opr == 'AND') {
                                    $subExpressions = array_filter($subExpressions, function ($se) {
                                        return $se != 'TRUE';
                                    });
                                }
                                if ($opr == 'AND' && empty($subExpressions)) {
                                    return 'TRUE';
                                }
                                if ($opr == 'AND' && in_array('FALSE', $subExpressions)) {
                                    return 'FALSE';
                                }
                                if ($opr == 'OR') {
                                    $subExpressions = array_filter($subExpressions, function ($se) {
                                ...
                                if ($opr == 'OR' && empty($subExpressions)) {
                                    return 'FALSE';
                                }
                                if ($opr == 'OR' && in_array('TRUE', $subExpressions)) {
                                    return 'TRUE';
                                }
                                ...
                                throw new Exception(
                                    "Invalid internal expression at operator `$opr`: " . json_encode($expression), 0, $e
                                );
                            }
                        case 3:
                        ...
                        case 5:
                            if (count($expression) != 1) {
                                throw new Exception("Operator `$opr` requires exactly one operand");
                            }
                            return $this->buildExpression($expression[0], $alias, $pr) . ' ' . $opr;
                        case 6:
                            if (count($expression) != 2) {
                                throw new Exception("Operator `$opr` requires exactly two operands.");
                            }
                            if ($opr == 'IN' && empty($expression[1])) {
                                return 'FALSE';
                            }
                            $r = $this->buildExpression($expression[0], $alias, $pr) .
                    ...
                    if (count($expression) > 1 && $pr > $precedence) {
                        $r = '(' . $r . ')';
                    }
                    return $r;
                }
        ...
        if (is_object($expression)) {
            return $this->connection->quoteValue($this->literal($expression));
        }
    ...
    public function buildOperator(string $op, array $expression, string $alias = null): string
    {
        ...
        if (!is_callable(array($this, $functionName))) {
            throw new Exception("Expression build function `$functionName` is missing");
        }
    ...
    public function buildJoin(
        ...
        if (!is_a($foreignModel, Model::class, true)) {
            throw new Exception("Model class `$foreignModel` is missing");
        }
        ...
        if ($hasOn) {
            ...
            if (empty($conditions)) {
                throw new Exception("Missing condition for $type JOIN");
            }
            ...
            return sprintf(
                ...
                $this->quoteSingleName(
                    $foreignTableName
                ) . ($alias && $alias != $foreignTableName ? ' ' . $this->quoteSingleName($alias) : ''),
                $this->buildExpression($expression, $mainAlias)
            );
        } else {
            if (!empty($conditions)) {
                throw new Exception("Needless condition for $type JOIN");
            }
            return sprintf(
                '%s JOIN %s',
                $type,
    ...
    {
        if (is_string($tables)) {
            return static::quoteFieldName($tables);
        }
    ...
    public function buildOrders(array|string|null $orders, string $alias = null): string
    {
        ...
        if (!is_array($orders)) {
            throw new Exception('Orders mut be an array');
        }
        if (!count($orders)) {
            return '';
        }
    ...
    {
        if (empty($o)) {
            throw new Exception('Empty order item.');
        }
        if (is_string($o)) {
            if (str_starts_with($o, '(')) {
                if (!str_ends_with($o, ')')) {
                    throw new Exception("Missing closing parenthesis in order item '$o'");
                }
                return substr($o, 1, -1);
            }
            if (str_contains($o, ' ')) {
        ...
        if (is_array($o)) {
            if (!isset($o[0])) {
                throw new Exception('Invalid order definition');
            }
            $fieldName = $o[0];
            ...
            try {
                return $preorder . $this->buildFieldName($fieldName, $alias) .
                    ...
                    (($this->connection->supportsOrderNullsLast() && ($nulls == self::NULLS_LAST || strtoupper(
                                $order
    ...
    {
        $alias = $this->normalizeAlias();
        $tablePart = $this->buildTableName(
            $this->modelClass
        );      // MySQL supports multiple table name here, not implemented (later)
    ...
    public function buildInsert(): string
    {
        ...
        if (!$fields && is_array($values) && ArrayHelper::isAssociative($values)) {
            ...
            if (!$fields) {
                throw new Exception('Must specify either `fields` or `values` for INSERT');
            }
            if (!ArrayHelper::isAssociative($fields)) {
                throw new Exception('`values` must contain the values or `fields` must be an associative array');
            }
            ...
            if (!($values instanceof Query) && ArrayHelper::isAssociative($values)) {
                throw new Exception(
                    '`values` must be a 2-dim numeric-indexed array of literal values of rows or a sub-Query'
                );
            }
    ...
    public function buildValues(Query|array|string|null $values, int $number = null): string
    {
        ...
        if (is_array($values)) {
            ...
            foreach ($values as $index => $valueList) {
                $result .= '(';
                if ($valueList === null) {
                    throw new Exception("#$index: Null values row.");
                }
                if (!is_array($valueList)) {
                    throw new Exception("#$index: Invalid values row. Values must contain arrays.", $valueList);
                }
                $n0 = count($valueList);
                $n = $number ?: $n0; // The actual number of values to be inserted
                if ($n0 > $n) {
                    throw new Exception("Too many values in the list #$index. $n0>$n", array($number, $valueList));
                }
                for ($i = 0; $i < $n; $i++) {
    ...
    public function buildFieldNames(array|string|null $models, ?array $fields, int|null &$number): string
    {
        ...
        if (ArrayHelper::isAssociative($models, false)) {
            $forceAlias = true;
        }
    ...
    {
        if ($fromPdo) {
            return implode('; ', $this->connection->pdo->errorInfo());
        }
     ...
     * @throws Exception
     */
    public function filterInReferred(
        ?string $value,
        string $field,
        Model|string $foreignClass,
        ...
        if ($value !== '' && $value !== null) {
            ...
            $candidatesQuery = $foreignClass::createQuery()
                ->where(['LIKE', ['lower()', $foreignValueField], $patternExpression],
                    [$patternName => $this->literal($value)])
