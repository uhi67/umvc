...
class Column extends Component {
    ...
    private static function createColumn($grid, $col, $model = null) {
        if(array_key_exists(0, $col)) $col['field'] = array_shift($col);
        if($model) $col['model'] = $model;
    ...
    public function init() {
        ...
        if($this->hint === null || $this->hint === true) {
            ...
            if($this->order) $this->hint .= ($this->hint ? '. ' : '') . 'Click to order by this column.';
        }
        /** Calculate label from model's attribute label */
        if($this->label === false) $this->label = '';
        elseif($this->label === null) $this->label = $originalLabel;
    ...
    public function renderSearch($search) {
        ...
        if(is_array($this->filter)) {
            $values = ['' => ''];
            foreach($this->filter as $value => $label) $values[$value] = $label; // array_merge messes up indices
            $result = Html::select($values, ['name' => $fieldName, 'class' => 'grid-filter'], $searchValue);
        } elseif($this->filter && $this->filter !== true) $result = $this->filter;
        else {
            ...
            $result = $this->field && $this->filter
                ...
                : '';
            if($this->searchIcon) {
                if($this->searchIcon === true) $this->searchIcon = '<i class="fa fa-search" style="display: table-cell; width:2rem;"></i>';
            ...
            if($this->searchCancel) {
                if($this->searchCancel === true) $this->searchCancel = '<i class="fa fa-times" style="display: table-cell; width:2rem;"></i>';
    ...
    public function renderValue(Model $model) {
        ...
        if(is_callable($this->value)) $content = call_user_func($this->value, $model);
        else {
        ...
        if(!is_scalar($content)) throw new Exception("Got " . gettype($content) . " at $this->field of " . get_class($model));
