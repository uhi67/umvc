...
class L10n extends Component
{
    ...
    public function prepare()
    {
        ...
        if ($this->switchParam && ($locale = App::$app->request->req($this->switchParam))) {
            $this->setUserLocale($locale);
        }

        if (!$this->source) {
            $this->source = App::$app->source_locale;
        }
        ...
        if ($this->supportedLocales && !$this->isSupported($this->locale)) {
            $this->locale = null;
        }
        if (!$this->locale) {
    ...
    {
        // 1. request parameter
        if ($this->switchParam && ($la = App::$app->request->req($this->switchParam)) && ($la = $this->isSupported(
                $la
            ))) {
            return $la;
        }
    ...
    public function getTextFile($cat, $dir, $source, $lang, $originalLanguage = null)
    {
        ...
        if (!isset(self::$_messages[$cat])) {
            self::$_messages[$cat] = [];
        }
        if (!isset(self::$_messages[$cat][$lang])) {
            $la = $lang;
            if (strlen($lang) == 5 && !file_exists($dir . '/' . $lang . '.php')) {
                $la = substr($lang, 0, 2);
            }
            $messagefile = $dir . '/' . $la . '.php';
            if (!file_exists($messagefile)) {
                if ($originalLanguage && substr($lang, 0, 2) == substr($originalLanguage, 0, 2)) {
                    return $source;
                }
        ...
        if (!isset(self::$_messages[$cat][$lang][$source])) {
            return $source . '*';
        }
